...
e2e-docker: ; docker run --rm --ipc=host --add-host=host.docker.internal:host-gateway -v "$$(pwd)":/work -w /work/apps/web -e PW_BASE_URL=http://host.docker.internal:3000 mcr.microsoft.com/playwright:v1.59.1-noble npx playwright test --reporter=list
